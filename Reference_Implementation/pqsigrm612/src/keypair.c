...
void export_sk(unsigned char *sk, matrix *Sinv, uint32_t *Q, 
	uint32_t *part_perm1, uint32_t* part_perm2, uint32_t *s_lead){
	//export private in order: Sinv, Q, part_perm1, pert_perm2, pivot of H_M
	export_matrix(sk        			, Sinv);
	
	memcpy		(sk+Sinv->alloc_size, 
									Q, sizeof(uint32_t)*CODE_N);
	memcpy		(sk+Sinv->alloc_size+sizeof(uint32_t)*CODE_N, 
								   part_perm1, sizeof(uint32_t)*CODE_N/4);

	memcpy		(sk+Sinv->alloc_size+sizeof(uint32_t)*CODE_N
		+sizeof(uint32_t)*CODE_N/4, 
								   part_perm2, sizeof(uint32_t)*(CODE_N-CODE_K));
	memcpy		(sk+Sinv->alloc_size+sizeof(uint32_t)*CODE_N
		+(sizeof(uint32_t)*CODE_N/4)*2, 
								   s_lead, sizeof(uint32_t)*(CODE_N-CODE_K));
}
...
crypto_sign_keypair(unsigned char *pk, unsigned char *sk){
	...
	uint32_t *part_perm1 = (uint32_t*)malloc(sizeof(uint32_t)*CODE_N/4);
	uint32_t *part_perm2 = (uint32_t*)malloc(sizeof(uint32_t)*CODE_N/4);

	uint32_t *Q = (uint32_t*)malloc(sizeof(uint32_t)*CODE_N);
	...
	uint32_t *s_lead = (uint32_t*)malloc(sizeof(uint32_t)*(CODE_N-CODE_K));
	uint32_t *s_diff = (uint32_t*)malloc(sizeof(uint32_t)*CODE_K);

	// uint32_t *p_lead = (uint32_t*)malloc(sizeof(uint32_t)*(CODE_N-CODE_K));
	// uint32_t *p_diff = (uint32_t*)malloc(sizeof(uint32_t)*CODE_K);
